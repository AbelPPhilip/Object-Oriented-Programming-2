...
public class TubeVideosManager implements TubeVideosManagerInt {
	...
	public boolean addVideoToDB(String title, String url, int durationInMinutes, Genre videoGenre) {
		if (title==null||title.isBlank()||url==null||url.isBlank()||durationInMinutes<0||videoGenre==null) {
			return false;
		}
	...
	public String[] getPlaylistsNames() {
		String [] playlistNames = new String[0];
	...
	public boolean addVideoToPlaylist(String title, String playlistName) {
		if (title==null|| playlistName==null || title.isBlank()||playlistName.isBlank()) {
			throw new IllegalArgumentException("Enter valid parameters");
		}
		if(playlists==null || videoDatabase==null ||playlists.isEmpty()||videoDatabase.isEmpty()) {
			return false;
		}
		if (findPlayListInternal(playlistName)==null || findVideo(title)==null) {
			return false;
		}
	    for (int i = 0;i<playlists.size();i++) {
	    	if(playlists.get(i).getName().equals(playlistName)) {
	    		playlists.get(i).addToPlaylist(title);
	    		return true;
	    	}
	    }
	    return false;
	...
	public Playlist getPlaylist(String playlistName) {
		if (playlistName==null||playlistName.isBlank()) {
			throw new IllegalArgumentException("Enter valid playlist name");
		}
		...
		for (int i = 0; i<playlists.size();i++) {
			if (playlists.get(i).getName().equals(playlistName)) {
				playlist = new Playlist(playlists.get(i));
				return playlist;
			}
		}
		return null; 
	...
	public void clearDatabase() {
		if (!(videoDatabase==null||videoDatabase.isEmpty()||playlists==null||playlists.isEmpty())) {
			videoDatabase.clear();
			playlists.clear();
		}
	...
	public Playlist searchForVideos(String playlistName, String title, int maximumDurationInMinutes, Genre genre) {
		if (playlists==null||playlists.isEmpty()||videoDatabase==null||videoDatabase.isEmpty()) {
			return null;
		}
		Playlist playlist = new Playlist(playlistName); 
		for (int i = 0; i<videoDatabase.size();i++) {
			...
			if (!(title==null||title.isBlank())){
				if (maximumDurationInMinutes>=vid.getDurationInMinutes()) {
					if(genre == null) {
						if (vid.getTitle().equals(title) && vid.getGenre().equals(genre)) {
							playlist.addToPlaylist(vid.getTitle());
						}
					}
					else {
